...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        #eps = golden(self.gold_func, args=(X, y, grad))
        eps = eta / len(mini_batch)
        self.weights = [w - eps * nw for w, nw in zip(self.weights, nabla_w)]
        self.biases = [b - eps * nb for b, nb in zip(self.biases, nabla_b)]

    def gold_func(self, alpha, X, y, grad):
        """
        Функция для одномерной минимизации методом золотого сечения
        """
        # assert w.shape[1] == 1

        return 0.5 * np.mean((self.activation_function(X.dot(self.w - alpha * grad)) - y) ** 2)

...
class SimpleCarWorld(World):
    ...
    HEADING_REWARD = 5 * 1e0
    WRONG_HEADING_PENALTY = 5 * 1e0
    IDLENESS_PENALTY = 32 * 1e-1
    SPEEDING_PENALTY = 0 * 1e-1
    MIN_SPEED = 0.1 * 1e0
    MAX_SPEED = 10 * 1e0
    '''
    COLLISION_PENALTY = 2 * 1e0
    ...
    def reward(self, state, collision):
        ...
        heading_reward = (-a+1) if a <= -0.5 else 1 if a <= -0.1 else 0
        heading_penalty = -1 if a > 0.2 else a if a > 0 else 0
    ...
    def run(self, steps=None):
        ...
        for i, agent in enumerate(self.agents):
            try:
                ...
                # pass
                print("File for writing not found")
    ...
    def vision_for(self, agent):
        ...
        :return: список из
        1) модуля скорости машинки,
        2) синуса направленного угла между направлением машинки и направлением на центр
        3) и `agent.rays` штук расстояний до ближайших стен трека (запустите картинку, и станет совсем понятно)
        """
    ...
    def visualize(self, scale):
        ...
        if len(self.agents) == 1:
            ...
            draw_text("Step: %.i" % a.step, self._info_surface, scale, self.size,
                      text_color=white, bg_color=black, tlpoint=(0, 10))
            draw_text("Cost_train %.3f" % a.cost_train, self._info_surface, scale, self.size,
                      text_color=white, bg_color=black, tlpoint=(0, 30))
            draw_text("Cost_test: %.3f" % a.cost_test, self._info_surface, scale, self.size,
                      text_color=white, bg_color=black, tlpoint=(0, 50))
            draw_text("Rewards_mean %.3f" % np.mean(a.reward_history), self._info_surface, scale, self.size,
                      text_color=white, bg_color=black, tlpoint=(0, 70))
    ...
    def _prepare_visualization(self):
        ...
        for state in self.agent_states.values():
            ...
            s.fill(white)
            pygame.draw.rect(s, red, pygame.Rect(0, 0, 15, 15))  # рисуем красный корпус машины
            pygame.draw.polygon(s, red, [(15, 0), (25, 8), (15, 15)], 0)  # рисуем  красный "наконечник" машины
...
if __name__ == "__main__":
    ...
    agent = SimpleCarAgent.from_file('network_config_agent_0_layers_9_6_2_1.txt')
    # # создаёт мир
